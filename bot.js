...
function isResourceDepleted(type, rx, ry, wx, wy) {
  ...
  if (type === 'gold' || type === 'diamond') {
    // If it is no longer in the active list, it has been mined/depleted.
    // Start counting missing ticks immediately (don't wait for attacks >= 2).
    if (state.lockResource && state.lockResource.type === type && state.lockResource.x === rx && state.lockResource.y === ry) {
...
function findBestResource(type) {
  ...
  if (type === 'gold' || type === 'diamond') {
    ...
    const candidates = list.filter(item => {
      ...
      // ALWAYS check cooldownResources (if we blacklisted it due to interference, it stays blacklisted!)
      if (state.cooldownResources && state.cooldownResources.has(key) && now < state.cooldownResources.get(key)) {
        return false;
      }
      ...
      // Avoid targeting nodes within 100px of another player to prevent stealing
      if (isAnotherPlayerMining(item.x, item.y, 100)) {
        return false;
      }
...
function doGold() {
  ...
  if (state.lockResource && state.lockResource.type === 'gold') {
    ...
    if (activeNode && activeNode.pct !== undefined && activeNode.pct === 0) {
      ...
      const isPlayerStealing = isAnotherPlayerMining(state.lockResource.x, state.lockResource.y, 100);
      ...
      if (interferenceMs >= 5000) {
        ...
        // User requested: If someone interferes for 5s, yield immediately. If 2x total, run far away.
        state._totalDisturbances = (state._totalDisturbances || 0) + 1;
        log('⚠️', `Emas [${state.lockResource.x},${state.lockResource.y}] direbut pemain lain (5 detik). Mengalah...`);
        
        if (!state.cooldownResources) state.cooldownResources = new Map();
        state.cooldownResources.set(countKey, Date.now() + 600000); // Blacklist node ini 10 menit
        state.lockResource = null;

        if (state._totalDisturbances >= 2) {
          log('🚫', `Sudah 2x diganggu di area ini! Mencari lokasi yang jauh...`);
          state._totalDisturbances = 0; // reset
          wander(600); // lari jauh
        } else {
          log('ℹ️', `Mencari emas lain di sekitar...`);
          wander(100); // geser dikit aja
        }
      }
    }
  }
