...
function log(icon, msg) {
  ...
  if (icon === 'DEBUG' || (icon === '??' && msg.includes('DEBUG'))) return;
  // Ignore annoying warnings about not registering or blacklisting
  if (msg.includes('not registering') || msg.includes('Blacklisting') || msg.includes('Re-aligning') || msg.includes('releasing lock')) return;
  
  const ts = new Date().toLocaleTimeString();
  if (typeof process !== 'undefined' && process.send && typeof sendToDashboard === 'function') {
    sendToDashboard({ type: 'log', payload: { ts, icon, msg } });
  } else {
...
const normalizeArray = (val) => {
  ...
  if (vals.length > 0 && Array.isArray(vals[0]) && Array.isArray(vals[0][0])) {
    return vals.flat();
  }
  return vals;
};
  ...
  switch (msg.t) {
    ...
    case 'farmobs':
      ...
      if (Array.isArray(msg.mobs)) {
        state.world.farMobs = normalizeArray(msg.mobs).map(m => ({ x: m[0], y: m[1], type: m[2] }));
      }
      break;
    ...
    case 'world':
      ...
      if (msg.mobs) {
              ...
              state.world.mobs = normalizeArray(msg.mobs).map(m => {
                if (Array.isArray(m)) {
                  return {
                    id: m[0],
                    x: m[1],
            ...
            if (msg.trees)    state.world.trees    = normalizeArray(msg.trees).map(t => Array.isArray(t) ? { x: t[0], y: t[1] } : t);
            if (msg.golds) {
              state.world.golds = normalizeArray(msg.golds).map(g => Array.isArray(g) ? { x: g[0], y: g[1], pct: (g[2] !== undefined && g[2] !== null) ? g[2] / 1000 : 1.0 } : g);
            }
            if (msg.diamonds) {
              state.world.diamonds = normalizeArray(msg.diamonds).map(d => Array.isArray(d) ? { x: d[0], y: d[1], pct: (d[2] !== undefined && d[2] !== null) ? d[2] / 1000 : 1.0 } : d);
            }
            if (msg.treeHits) state.world.treeHits = normalizeArray(msg.treeHits).map(h => Array.isArray(h) ? { x: h[0], y: h[1], pct: h[2] / 1000 } : h);
      if (msg.players) {
        state.world.players = normalizeArray(msg.players).map(p => {
          if (Array.isArray(p)) {
            return {
              id: p[0],
              x: p[1],
        ...
        const dangerousPlayer = state.world.players.find(p => p.name && p.name.toLowerCase().includes('lawyercat'));
        if (dangerousPlayer && !state._isDisconnecting) {
          state._isDisconnecting = true;
          const delayMs = Math.floor(Math.random() * (20000 - 10000 + 1)) + 10000;
          log('🚨', `DANGER! ${dangerousPlayer.name} detected! Simulating human delay... disconnecting in ${Math.round(delayMs/1000)}s`);
          setTimeout(() => {
            log('🚨', `Disconnecting now to avoid ban from ${dangerousPlayer.name}!`);
            process.exit(0);
          }, delayMs);
        }
...
function isResourceDepleted(type, rx, ry, wx, wy) {
  ...
  if (type === 'gold' || type === 'diamond') {
    // Kita harus memukul emasnya minimal 1 kali agar server merespon dan memunculkan emasnya di daftar aktif.
    // Jika kita belum memukulnya, kita tidak bisa menyimpulkan bahwa emas itu kosong/habis.
    if (state.lockResource && state.lockResource.type === type && state.lockResource.x === rx && state.lockResource.y === ry && state.attacksOnCurrentTarget >= 1) {
      ...
      if (!exists) {
        ...
        // Tunggu sebentar (15 tick / 1.5 detik) setelah dipukul. Jika tidak muncul juga, berarti memang kosong/cooldown.
        if (state._missingActiveTicks >= 15) {
...
function isAnotherPlayerMining(gx, gy, checkRange = 90) {
  ...
  const playersInRange = state.world.players.filter(p => {
    if (!isHumanPlayer(p)) return false;
    const d = Math.hypot(p.x - wx, p.y - wy);
    return d < checkRange;
  });

  if (playersInRange.length === 0) return false;

  // Bedakan pemain yang sedang diam (AFK) dan menyerang.
  // Jika pct < 1.0, berarti emas sedang ditambang/diserang.
  const activeNode = state.world.golds.find(g => g.x === gx && g.y === gy) || state.world.diamonds.find(d => d.x === gx && d.y === gy);
  const isDamaged = activeNode && activeNode.pct !== undefined && activeNode.pct < 1.0;
...
function findBestResource(type) {
  ...
  const isNempel = state.world.players.some(p => p.id !== state.player.id && isHumanPlayer(p) && Math.hypot(p.x - state.player.x, p.y - state.player.y) < 100);
  ...
  if (type === 'gold' || type === 'diamond') {
    ...
    const candidates = list.filter(item => {
      ...
      // Jika emas tersebut sudah kita tahu sedang cooldown dari server (misal pct === 0)
      if (activeNode && activeNode.pct === 0) {
        if (!state.cooldownResources) state.cooldownResources = new Map();
        state.cooldownResources.set(key, now + 60000);
        return false;
      }
    ...
    for (const item of finalCandidates) {
      ...
      // DITERAPKAN KE SEMUA JARAK agar bot-bot yang berada berdekatan tidak saling bertumpuk (berpencar).
      ...
      const localDispersion = ((uniqueHash ^ nodeHash) % 8) * 200; // Penalty 0 - 1400px (acak untuk tiap bot)
...
function doGold() {
  ...
  if (state.lockResource && state.lockResource.type === 'gold') {
    ...
    if (activeNode && activeNode.pct !== undefined && activeNode.pct === 0) {
      ...
      // Only count as 1 disturbance event after 3 continuous seconds of interference
      const interferenceMs = state._playerNearNodeTime ? Date.now() - state._playerNearNodeTime : 0;
      if (interferenceMs >= 3000) {
        state._playerNearNodeTime = 0; 
        const countKey = `gold:${state.lockResource.x},${state.lockResource.y}`;
        
        if (state.attacksOnCurrentTarget > 0) {
          // Kita yang sedang menambang, JANGAN mengalah jika ada yang mendekat!
          // Tetap pertahankan emas ini.
          state._nodeDisturbances = state._nodeDisturbances || {};
          state._nodeDisturbances[countKey] = 0;
        } else {
          log('ℹ️', `Menghindari rebutan emas di [${state.lockResource.x},${state.lockResource.y}] dengan pemain lain. Mencari target lain...`);
          if (!state.cooldownResources) state.cooldownResources = new Map();
          state.cooldownResources.set(countKey, Date.now() + 60000); // Blacklist 1 menit agar tidak kembali ke sini
          state.lockResource = null;
          wander(250);
          return;
        }
  ...
  let walkTimeoutMs = Math.min(45000, Math.max(10000, Math.round(d * 15)));
  if (state.lockResource && state.lockResource.type === 'gold') walkTimeoutMs = 120000; // 2 minutes for gold to prevent random drops
...
function doMonster(bossOnly = false) {
  ...
  if (state.player.hpPct !== undefined) {
    if (state.player.hpPct < 0.20 && !state.isRegenerating) {
...
function attackTick() {
  ...
  if (state.customTarget) {
    const d = distTo(state.customTarget.x, state.customTarget.y);
    if (d < 10) {
      log('✅', `Arrived at destination (${state.customTarget.x}, ${state.customTarget.y}).`);
      state.customTarget = null;
    } else {
      moveToward(state.customTarget.x, state.customTarget.y, 80, 0);
      return;
    }
  }
  ...
  if (state.world.mobs && state.player.hpPct > 0) {
    const aggroMob = state.world.mobs.find(m => {
      ...
      const defRange = state.modeList.includes('monster') ? 250 : 150; // Naikkan jarak agresif
      return d < defRange; 
    });
...
async function preloadAllChunks() {
  ...
  for (let cy = 0; cy <= 9; cy++) {
    for (let cx = 0; cx <= 9; cx++) {
      ...
      await sleep(150); // Tambah jeda agar server tidak spam (rate limit)
    }
  }
...
function connect(sessionToken) {
  ...
  state.ws.on('close', async (code) => {
    ...
    let tok = null;
    for (let attempt = 1; attempt <= 999999; attempt++) {
      try {
        tok = await doLogin();
        break;
      } catch (e) {
        log('⏳', `Login failed (${e.message}) — waiting 10s (attempt ${attempt})...`);
        await sleep(10000);
      }
    }
...
if (typeof process !== 'undefined') {
  process.on('message', (msg) => {
    if (msg.type === 'command' && msg.command === 'goto') {
      state.customTarget = { x: msg.x, y: msg.y };
      log('🗺️', `Directed to move to (${msg.x}, ${msg.y})`);
    }
  });
}
