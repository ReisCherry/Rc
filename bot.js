...
function isResourceDepleted(type, rx, ry, wx, wy) {
  ...
  if (type === 'gold' || type === 'diamond') {
    ...
    // Jangan pukul angin kosong! Jika node tidak ada di server dan jarak dekat (< 320px)
    if (!exists && d < 320) {
      state._missingActiveTicks = (state._missingActiveTicks || 0) + 1;
      const threshold = (state.attacksOnCurrentTarget > 0) ? 10 : 3;
      if (state._missingActiveTicks >= threshold) {
