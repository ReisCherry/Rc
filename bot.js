...
function doMonster(bossOnly = false) {
  ...
  // Aktifkan mode regen jika HP kritis (< 30%) DAN dikeroyok >= 3 monster
  if (state.player.hpPct !== undefined) {
    if (state.player.hpPct < 0.3 && !state.isRegenerating) {
      const nearbyAggressive = alive.filter(m => distTo(m.x, m.y) < 150);
      if (nearbyAggressive.length >= 3) {
        state.isRegenerating = true;
        log('🏃', `HP Kritis (${Math.round(state.player.hpPct*100)}%) dan dikeroyok! Masuk ke mode JAGA JARAK untuk regen...`);
      }
    } else if (state.player.hpPct >= 0.7 && state.isRegenerating) {
      state.isRegenerating = false;
      log('💖', `HP sudah pulih (${Math.round(state.player.hpPct*100)}%). Kembali berburu!`);
    }
  }
...
function attackTick() {
  ...
  if (state.world.mobs && state.player.hpPct > 0) {
    const aggroMob = state.world.mobs.find(m => {
      ...
      return d < 55; // Aggro/defense range: reduced to 55px so we only defend if they are directly next to us/hitting us
    });
  ...
  if (state.playerPosInitialized && !state.initialDispersionDone) {
    if (!state.dispersionTarget) {
      const nameHash = hashCode(WALLET + (BOT_NAME || ''));
      const angle = (nameHash % 8) * (Math.PI / 4);
      const dispersionDistance = 300 + (nameHash % 5) * 50;
