...
function checkRealign(key, near) {
  ...
  if (key.startsWith('mob:')) {
    ...
    timeoutMs = 45000; // 45 seconds timeout jika HP sama sekali tidak turun (menghindari nyangkut di ghost node)
    minAttacks = 60;
  }
...
function findBestResource(type) {
  ...
  if (type === 'gold' || type === 'diamond') {
    ...
    const candidates = list.filter(item => {
      ...
      // Avoid targeting nodes close to the last disturbed location (within 1000px)
      if (type === 'gold' && state._lastDisturbedPos && now - state._lastDisturbedPos.time < 600000) {
        const distToDisturbed = Math.hypot(item.wx - state._lastDisturbedPos.x, item.wy - state._lastDisturbedPos.y);
        if (distToDisturbed < 1000) {
          return false;
        }
      }
      ...
      // EXCEPT if the node is already damaged (low health) or very close to us (we are already standing next to it)
      const isDamaged = activeNode && activeNode.pct !== undefined && activeNode.pct < 0.95;
      const isVeryClose = d < 120;
      if (!isDamaged && !isVeryClose && isAnotherPlayerMining(item.x, item.y, 75)) {
        return false;
      }
      ...
      if (d <= 320) {
        ...
        if (!isExistsAndActive) {
          if (!state.cooldownResources) state.cooldownResources = new Map();
          state.cooldownResources.set(key, now + 300000); // 5 minutes cooldown
          return false;
        }
      }
    ...
    for (const item of finalCandidates) {
      ...
      const localDispersion = d <= 400 ? 0 : ((nameHash ^ nodeHash) % 6) * 300; // 0 to 1800px penalty
